...
export class LocationServiceService {
  ...
  fetchApi():Observable<Dia>{
    return this.http.get<ApiCall>(this.apiUrl+'?api_key='+this.token).pipe(
      switchMap((api: ApiCall)=>{
        const apiUrl2= api.datos;
        return this.http.get<Dia>(apiUrl2);
      }),
      catchError((error)=>{
